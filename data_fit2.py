...
# number of contacts n, improved fit pq by doing p then q or q then p,
# and by not modeling the q variation
# 06/07/2016
# Taisuke Yasuda
#########################################################################
...
colors = {'pq':'green','px':'blue','xq':'yellow','xx':'red','ptq':'pink','qtp':'orange'}
extract_pool = {
...
class FitDist:
    ...
    def simulate(self):
        ...
        p, q = self.p, self.q
        if (self.model.name[0] == 'x' or type(p) == float or type(p) == np.float64):
            p = [self.p]*Cell.N
        if (self.model.name[1] == 'x' or type(q) == float or type(q) == np.float64):
            q = [self.q]*Cell.N
        #### simulation ####
        hist = None
        responses = list()
...
for model in models:
    ...
    for cell in cells:
        ...
        for num_trials in trials:
            ...
            p_opt = None
            #### fit qtp is fitting q first, then fitting p ####
            #### fit ptq is fitting p first, then fitting q ####
            p,q = list(),list()
            for i in range(Cell.N):
                #### initial probabilities are normally distributed ####
                temp = normal(fit.avg_p,FitDist.STDVAR_P)
                temp = min(max(temp,0),1)
                p.append(temp)
                #### initial responses are normally distributed ####
                temp = normal(fit.avg_q,FitDist.STDVAR_Q)
                temp = max(temp,0)
                q.append(temp)
            if (model.name == 'qtp'):
                #### fit q first ####
                p0 = q
                def obj_func(x,q0,q1,q2,q3,q4,q5,q6,q7,q8,q9):
                    fit.p = fit.avg_p
                    fit.q = [q0,q1,q2,q3,q4,q5,q6,q7,q8,q9]
                    result = fit.simulate()
                    return eval_hist(result,x)
                lo_bound = [0]*Cell.N
                up_bound = [np.inf]*Cell.N
                bounds = (lo_bound,up_bound)
                p_opt,p_cov = opt.curve_fit(obj_func,xdata,ydata,p0=p0,bounds=bounds)
                #### then fit p fit p_opt as q ####
                p0 = p
                fit.q = p_opt
                def obj_func(x,p0,p1,p2,p3,p4,p5,p6,p7,p8,p9):
                    fit.p = [p0,p1,p2,p3,p4,p5,p6,p7,p8,p9]
                    result = fit.simulate()
                    return eval_hist(result,x)
                lo_bound = [0]*Cell.N
...
vspace = 50
label = 20
img_height = 4*vspace + 4*height + title
img_width = 3*hspace + 2*width + 2*label
...
mid = top+height+vspace
bot = mid+height+vspace
left = label+hspace
right = label+hspace+width+hspace

box = {'pq':(left,top), 'px':(right,top), 'xq':(left,mid), 'xx':(right,mid),
        'ptq':(left,bot),'qtp':(right,bot)}
...
nrmsqe = {'pq':(left,top), 'px':(right,top), 'xq':(left,mid), 'xx':(right,mid),
        'ptq':(left,bot),'qtp':(right,bot)}
...
bot -= height/2
left = 10
right = label + width + hspace + width + hspace + 10

label = {'pq': (left, top), 'px': (right, top), 'xq': (left,mid), 'xx': (right, mid),
        'ptq': (left, bot), 'qtp': (right, bot)}
    ...
    for cell in cells:
        for zoom in [True,False]:
            ...
            for model in models:
                ...
                draw.text(label[model.name],model.name,black,font=l_font)
...
for num_trial in trials:
    ...
    ax.set_xticklabels(('pq','xq','px','xx','ptq','qtp'))
